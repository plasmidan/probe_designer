...
def downloadNcbiGenomes(accessions: dict, genome_database_dir: str, download_mode: str = 'basic', is_unzip: bool = True,
    ...
    """ Downloading genome files from NCBI assembly databse.
        Process GFF and fasta files for downstream analysis
    """
...
def processDownloadedGenome(downloaded_genome_path: str, accession: str, is_img = False):
    """"
    Allows for NCBI or Integrated Microbial Genome (IMG) downloaded entries
    Generates the files required for downstream analysis
    """
    # generate a simple genes annotation file from gff
    parseGffFile(downloaded_genome_path, accession, is_img) 
    
    # merge and rename using locus_tag;contig notation
    mergeAndRewriteFasta(downloaded_genome_path, accession,is_img) 
...
def getGenomeUrl(assembly_name: str, email: str, is_refseq_format: bool = True) -> str:
    ...
    provides an option to download the RefSeq or GenBank versions
    """
...
def parseGffFile(genome_dir: str, accession: str,is_img = False) -> None:
    """Create more readable version for the genes info."""
    
    data = []
    gff_file = glob.glob(os.path.join(genome_dir, f"*{'.gff'}"))
    if not is_img:
        allowed_line_options = ['gene']
    else:
        allowed_line_options = ['CDS','RNA','rRNA','tRNA','ncRNA']
    with open(rf'{gff_file[0]}', 'r') as f:
        ...
        while True:
            ...
            try:
                if line_1_sp[2] in allowed_line_options:
                    if not is_img:
                        line2 = f.readline().rstrip()
                        line_2_sp = line2.rstrip().split('\t')
                    else:
                        line_2_sp = line_1_sp.copy()
...
def extractCdsInfo(genes_info_df,gff_file_path, output_file_path):
    """ Generates an exon/CDS file for each gene
        file later used for extracting exon specific probes
    """
