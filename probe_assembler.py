...
class ProbeAssembler:
    "Combining primary probes with readouts, linkers, primers, etc."""

    def __init__(self, run_dir: str, primary_probes: str, selected_gene_list: str, probe_per_gene: str,
                 is_amplification: bool = True, num_of_flanking: int = 2, 
                 fwd_primer: str = 'TTTCGTCCGCGAGTGACCAG', rev_primer_revcom: str = 'CAACGTCCATGTCGGGATGC',t7_promoter_revcom: str = 'CCCTATAGTGAGTCGTATTA',
                 max_probes_per_ch: dict = {'fluor_640nm': 25, 'fluor_560nm': 25, 'fluor_488nm': 30},
        ...
        self.run_dir = run_dir
        self.ro_df = getRoDf(probes_to_remove=probes_to_remove) #LO
        ...
        self.probes_to_replace = probes_to_remove #LO 
        self.genes_of_interest = genes_of_interest 
        self.reference_genes = reference_genes #LO

        self.available_probes = len(self.ro_df) #
        self.positive_control_genes = positive_control_genes #LO
        self.negative_control_genes = negative_control_genes #LO
        self.used_readouts = []
    ...
    def readFiles(self) -> None:
        ...
        #read gene list (mandetory locus_tag column)
        ...
        self.all_genes_df['locus_tag'] = self.all_genes_df['locus_tag'].str.strip()


        # get rows and probe counts
        ref_gene_df = self.probe_per_gene[self.probe_per_gene['locus_tag'].isin(self.reference_genes)] #LO
        self.ref_gene_df = ref_gene_df.sort_values(by='probe_count', ascending=True) #LO

        self.all_genes_df = self.probe_per_gene[self.probe_per_gene['locus_tag'].isin(self.all_genes_df['locus_tag'])] #LO
        self.ref_gene_df = self.ref_gene_df.sort_values(by='probe_count', ascending=True) #LO
    ...
    def prioritizeAndPartitionGenes(self) -> None:
        """Prioritize and set channels.
           deal with cases where the number of genes does not divide by 3
        """
        
        genes_of_interest_df = self.all_genes_df[self.all_genes_df['locus_tag'].isin(self.genes_of_interest)]
        self.all_genes_df = pd.concat([genes_of_interest_df, self.all_genes_df[~self.all_genes_df['locus_tag'].isin(self.genes_of_interest)]]) #prioritize genes of interest in A647 channel

        genes_part_size = len(self.all_genes_df) // 3 #
        genes_to_parts = self.all_genes_df.iloc[:genes_part_size*3]
        genes_leftovers = self.all_genes_df.iloc[genes_part_size*3:]

        fluor_640nm_genes = genes_to_parts.iloc[:genes_part_size]
        fluor_560nm_genes = genes_to_parts.iloc[genes_part_size:2*genes_part_size]
        fluor_488nm_genes = genes_to_parts.iloc[2*genes_part_size:]

        if len(genes_leftovers) == 1:
            fluor_640nm_genes = pd.concat([fluor_640nm_genes,genes_leftovers])
        elif len(genes_leftovers) == 2:
            fluor_640nm_genes = pd.concat([fluor_640nm_genes,genes_leftovers.iloc[[0]]])
            fluor_560nm_genes = pd.concat([fluor_560nm_genes,genes_leftovers.iloc[[1]]])
        else:
            pass

        self.channel_sets = {
            'fluor_640nm': fluor_640nm_genes,
            'fluor_560nm': fluor_560nm_genes,
            'fluor_488nm': fluor_488nm_genes
        }

        return None

    #DAR LAB ONLY
    def prioritizeAndPartitionGenesRef(self) -> None:
        """Prioritize and set channels."""
    ...
    def assembleProbes(self) -> None:
        ...
        for ch, df in self.channel_sets.items():
            ...
            for i in range(len(df)):
                ...
                probe_num = min(int(num_of_possible_probes), int(self.max_probes_per_ch[ch]))
    ...
    def saveOutput(self) -> None:
        ...
        output_name = re.sub('.xlsx','',self.selected_gene_list)
    ...
    def design(self) -> None:
        ...
        if len(self.reference_genes) == 0:
            self.prioritizeAndPartitionGenes()
        else:
            self.prioritizeAndPartitionGenesRef() 
        self.assembleProbes()
        try:
            self.labelGeneClass()
        except(Exception,):
